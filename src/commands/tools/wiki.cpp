...
void setup_wiki( CLI::App& app )
{
    ...
    auto sub = app.add_subcommand(
        "wiki",
        "Create wiki help pages for spear."
    )->group( "" );
    ...
    auto md_dir_opt = sub->add_option(
        "--md-dir",
        options->md_dir,
        "Directory with the Markdown files documenting the spear commands."
    );
...
void add_wiki_command_header_synopsis(
    ...
    os << "Documentation for spear " << spear_version() << "\n\n";
}
...
void make_options_table( WikiOptions const& wiki_options, CLI::App const& command, std::ostream& os )
{
    ...
    for( auto const& opt : options ) {
        ...
        if( opt->get_name() == "--threads" ) {
            ...
            if( !search.empty() ) {
                opt_str = replace_all( opt_str, search, tn );
            }
        }
        ...
        tmp_os << "<dd>";
        if( ! opt_str.empty() ) {
            tmp_os << "<code>" << trim( opt_str ) << "</code><br />";
        }
        // tmp_os << "</dt>\n";
