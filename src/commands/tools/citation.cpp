...
{
    // Good place to check citations. This is executed every time with spear,
    ...
    auto sub = app.add_subcommand(
        "citation",
        "Print references to be cited when using spear."
    );
    ...
    sub->add_flag(
        "--all",
        options->all,
        "Print all relevant citations used by commands in spear."
    );
    sub->add_flag(
        "--list",
        options->list,
...
void run_citation( CitationOptions const& options )
{
    ...
    // If not, by default, just print the spear reference itself.
